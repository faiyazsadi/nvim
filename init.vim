
source ~/AppData/Local/nvim/vim-plug/plugins.vim
source ~/AppData/Local/nvim/general/settings.vim
source ~/AppData/Local/nvim/general/quickfix.vim
source ~/AppData/Local/nvim/keys/mappings.vim
source ~/AppData/Local/nvim/plug-config/vim-startify.vim
source ~/AppData/Local/nvim/plug-config/vim-floaterm.vim
source ~/AppData/Local/nvim/plug-config/vim-cpp-modern.vim
source ~/AppData/Local/nvim/themes/airline.vim
source ~/AppData/Local/nvim/snippets/snippets.vim
source ~/AppData/Local/nvim/comp/cp.vim

"lua require 'init'
"lua require 'clangd'
"lua require 'treesitter'
